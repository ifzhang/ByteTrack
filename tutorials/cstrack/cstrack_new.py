...
#         pred = pred[pred[:, :, 4] > self.opt.conf_thres]
        pred = pred[pred[:, :, 4] > self.det_thresh_low]
        detections = []
        dets_second = []
        scores_second = []
        id_feature_second = []
...
#             dets,x_inds,y_inds = non_max_suppression_and_inds(pred[:,:6].unsqueeze(0), self.opt.conf_thres, self.opt.nms_thres,method='cluster_diou')
            dets,x_inds,y_inds = non_max_suppression_and_inds(pred[:,:6].unsqueeze(0), self.det_thresh_low, self.opt.nms_thres, method='cluster_diou')
            if len(dets) != 0:
                ...
                inds_low = scores > self.det_thresh_low
...
#                                   det_features, cost_matrix, cost_matrix_det, cost_matrix_track, max_num=5, out_path=save_dir)
        ...
        ''' Step 3plus: Second association with IOU, association the untrack to the low score detections'''
